...
loadDistScaledData <- function() {
  
  IDs <- findParticipants(task='distScaled')
  ...
  for (ID in IDs) {
    ...
    participantData <- loadDistScaledParticipant(ID)
...
findTaskFiles <- function(ID, task) {
  ...
  if (task %in% c('distHorizontal', 'distScaled')) {
    runs <- c('LH','RH')
  }
...
loadDistScaledParticipant <- function(ID) {
  
  files <- findTaskFiles(ID, 'distScaled')
...
processDistScaledData <- function() {
  
  allData <- loadDistScaledData()
...
plotDistScaledPsychometricCurve <- function(target='inline', cluster=NULL) {
  ...
  setupFigureFile( target = target, 
                   ...
                   height = 5, 
                   dpi = 300, 
                   filename = sprintf('doc/fig/%s/dist_scaled_psychometric.%s', target, target))
  
  
  allConditionData <- processDistScaledData()
  ...
  plot(-1000,-1000,
       xlim=c(-1.1, 1.1), ylim=c(0,1),
       main=sprintf('scaled distance perception (N=%d)',N), xlab='difference [dva]', ylab='proportion target chosen',
       ax=F, bty='n')
  
  lines(x=c(-1.05,1.05),y=c(0.5,0.5),col='gray',lty=3)
